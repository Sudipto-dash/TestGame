...
public class CanvasCollider : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject OpponentUI;
    [SerializeField]
    TMPro.TextMeshProUGUI trade;
    [SerializeField]
    TMPro.TextMeshProUGUI follow;
    [SerializeField]
    TMPro.TextMeshProUGUI profile;
    [SerializeField]
    ...
    private void Update()
    {
       ...
       if(playerControls.Player.Chat.triggered || playerControls.Player.Chat.ReadValue<float>()>0)
       {
           Debug.Log("Chat UI is pressed");
       }
      if(playerControls.Player.Backpack.triggered || playerControls.Player.Backpack.ReadValue<float>()>0)
       {
        ...
        if (other.tag == "Player")
        {
            ...
            if (GameIDOpponent != playerName) {
                ...
                if (CanvasTrading.active == false)
                {
                   ...
                   Debug.Log(" Set Active Texts");
                   OpponentUI.SetActive(true);
                   
                   trade.text = "Trade with "+GameIDOpponent;
                   follow.text = "Follow "+GameIDOpponent;
                   profile.text = "Lookup "+GameIDOpponent;
